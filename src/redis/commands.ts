...
async function executeRaw(
  ...
  switch (cmd) {
    case "PING":
      return { result: await store.ping(a[0]) };
    case "ECHO": {
      const err = exactArgs(a, 1, "echo");
      if (err) return { error: err };
      return { result: await store.echo(a[0]!) };
    }
    case "TIME":
      return { result: await store.time() };
    case "SET": {
      const err = minArgs(a, 2, "set");
      if (err) return { error: err };
      const opts = parseSetOptions(a.slice(2));
      if (typeof opts === "string") return { error: opts };
      const result = await store.set(a[0]!, a[1]!, opts);
    ...
    case "GET": {
      ...
      if (err) return { error: err };
      return { result: await await store.get(a[0]!) };
    }
    case "MSET": {
    ...
    case "MGET": {
      ...
      if (err) return { error: err };
      return { result: await await store.mget(a) };
    }
    case "SETNX": {
    ...
    case "SETEX": {
      ...
      await store.setex(a[0]!, seconds, a[2]!);
      return { result: "OK" };
    }
    case "PSETEX": {
      ...
      await store.psetex(a[0]!, ms, a[2]!);
      return { result: "OK" };
    }
    case "INCR": {
      const err = exactArgs(a, 1, "incr");
      if (err) return { error: err };
      return { result: await await store.incr(a[0]!) };
    }
    case "INCRBY": {
      ...
      return { result: await await store.incrby(a[0]!, inc) };
    }
    case "INCRBYFLOAT": {
      ...
      return { result: await await store.incrbyfloat(a[0]!, inc) };
    }
    case "DECR": {
      const err = exactArgs(a, 1, "decr");
      if (err) return { error: err };
      return { result: await await store.decr(a[0]!) };
    }
    case "DECRBY": {
      ...
      return { result: await await store.decrby(a[0]!, dec) };
    }
    case "APPEND": {
      const err = exactArgs(a, 2, "append");
      if (err) return { error: err };
      return { result: await await store.append(a[0]!, a[1]!) };
    }
    case "STRLEN": {
      const err = exactArgs(a, 1, "strlen");
      if (err) return { error: err };
      return { result: await await store.strlen(a[0]!) };
    }
    case "GETRANGE": {
      ...
      return { result: await await store.getrange(a[0]!, start, end) };
    }
    case "SETRANGE": {
      ...
      return { result: await await store.setrange(a[0]!, offset, a[2]!) };
    }
    case "GETDEL": {
      const err = exactArgs(a, 1, "getdel");
      if (err) return { error: err };
      return { result: await await store.getdel(a[0]!) };
    }
    case "GETEX": {
      ...
      if (typeof opts === "string") return { error: opts };
      return { result: await await store.getex(a[0]!, opts) };
    }
    case "DEL": {
      const err = minArgs(a, 1, "del");
      if (err) return { error: err };
      return { result: await await store.del(a) };
    }
    case "UNLINK": {
      const err = minArgs(a, 1, "unlink");
      if (err) return { error: err };
      return { result: await await store.unlink(a) };
    }
    case "EXISTS": {
      const err = minArgs(a, 1, "exists");
      if (err) return { error: err };
      return { result: await await store.exists(a) };
    }
    case "EXPIRE": {
    ...
    case "TTL": {
      ...
      if (err) return { error: err };
      return { result: await await store.ttl(a[0]!) };
    }
    case "PTTL": {
      const err = exactArgs(a, 1, "pttl");
      if (err) return { error: err };
      return { result: await await store.pttl(a[0]!) };
    }
    case "PERSIST": {
    ...
    case "RENAME": {
      ...
      if (err) return { error: err };
      await store.rename(a[0]!, a[1]!);
      return { result: "OK" };
    }
    case "TYPE": {
      const err = exactArgs(a, 1, "type");
      if (err) return { error: err };
      return { result: await await store.type(a[0]!) };
    }
    case "KEYS": {
      const err = exactArgs(a, 1, "keys");
      if (err) return { error: err };
      return { result: await await store.keys(a[0]!) };
    }
    case "SCAN": {
      ...
      const [next, keys] = await store.scan(cursor, opts);
      return { result: [String(next), keys] };
    }
    case "DBSIZE":
      return { result: await await store.dbsize() };
    case "FLUSHDB": {
      await store.flushdb();
      return { result: "OK" };
    }
    ...
    case "RANDOMKEY":
      return { result: await await store.randomkey() };
    case "COPY": {
    ...
    case "HSET": {
      ...
      return { result: await await store.hset(a[0]!, fields) };
    }
    case "HGET": {
      const err = exactArgs(a, 2, "hget");
      if (err) return { error: err };
      return { result: await await store.hget(a[0]!, a[1]!) };
    }
    case "HMSET": {
      ...
      await store.hmset(a[0]!, fields);
      return { result: "OK" };
    }
    case "HMGET": {
      const err = minArgs(a, 2, "hmget");
      if (err) return { error: err };
      return { result: await await store.hmget(a[0]!, a.slice(1)) };
    }
    case "HGETALL": {
      const err = exactArgs(a, 1, "hgetall");
      if (err) return { error: err };
      return { result: await await store.hgetall(a[0]!) };
    }
    case "HDEL": {
      const err = minArgs(a, 2, "hdel");
      if (err) return { error: err };
      return { result: await await store.hdel(a[0]!, a.slice(1)) };
    }
    case "HEXISTS": {
    ...
    case "HLEN": {
      ...
      if (err) return { error: err };
      return { result: await await store.hlen(a[0]!) };
    }
    case "HKEYS": {
      const err = exactArgs(a, 1, "hkeys");
      if (err) return { error: err };
      return { result: await await store.hkeys(a[0]!) };
    }
    case "HVALS": {
      const err = exactArgs(a, 1, "hvals");
      if (err) return { error: err };
      return { result: await await store.hvals(a[0]!) };
    }
    case "HINCRBY": {
      ...
      const inc = toInt(a[2]!);
      if (inc === null) return { error: "ERR value is not an integer or out of range" };
      return { result: await await store.hincrby(a[0]!, a[1]!, inc) };
    }
    case "HINCRBYFLOAT": {
      ...
      if (inc === null) return { error: "ERR value is not a valid float" };
      return { result: await await store.hincrbyfloat(a[0]!, a[1]!, inc) };
    }
    case "HSETNX": {
    ...
    case "HSCAN": {
      ...
      const [next, items] = await store.hscan(a[0]!, cursor, opts);
    ...
    case "LPUSH": {
      ...
      if (err) return { error: err };
      return { result: await await store.lpush(a[0]!, a.slice(1)) };
    }
    case "RPUSH": {
      const err = minArgs(a, 2, "rpush");
      if (err) return { error: err };
      return { result: await await store.rpush(a[0]!, a.slice(1)) };
    }
    case "LPOP": {
      ...
      if (a.length > 1) {
        ...
        if (count === null || count < 0)
          return { error: "ERR value is not an integer or out of range" };
        return { result: await await store.lpop(a[0]!, count) };
      }
      return { result: await await store.lpop(a[0]!) };
    }
    case "RPOP": {
      ...
      if (a.length > 1) {
        ...
        if (count === null || count < 0)
          return { error: "ERR value is not an integer or out of range" };
        return { result: await await store.rpop(a[0]!, count) };
      }
      return { result: await await store.rpop(a[0]!) };
    }
    case "LRANGE": {
      ...
      if (start === null || stop === null)
        return { error: "ERR value is not an integer or out of range" };
      return { result: await await store.lrange(a[0]!, start, stop) };
    }
    case "LLEN": {
      const err = exactArgs(a, 1, "llen");
      if (err) return { error: err };
      return { result: await await store.llen(a[0]!) };
    }
    case "LINDEX": {
      ...
      return { result: await await store.lindex(a[0]!, index) };
    }
    case "LSET": {
      ...
      const index = toInt(a[1]!);
      if (index === null) return { error: "ERR value is not an integer or out of range" };
      await store.lset(a[0]!, index, a[2]!);
      return { result: "OK" };
    }
    case "LINSERT": {
      ...
      return { result: await await store.linsert(a[0]!, dir === "BEFORE", a[2]!, a[3]!) };
    }
    case "LREM": {
      ...
      return { result: await await store.lrem(a[0]!, count, a[2]!) };
    }
    case "LTRIM": {
      ...
      if (start === null || stop === null)
        return { error: "ERR value is not an integer or out of range" };
      await store.ltrim(a[0]!, start, stop);
      return { result: "OK" };
    }
    ...
    case "SADD": {
      ...
      if (err) return { error: err };
      return { result: await await store.sadd(a[0]!, a.slice(1)) };
    }
    case "SREM": {
      const err = minArgs(a, 2, "srem");
      if (err) return { error: err };
      return { result: await await store.srem(a[0]!, a.slice(1)) };
    }
    case "SMEMBERS": {
      const err = exactArgs(a, 1, "smembers");
      if (err) return { error: err };
      return { result: await await store.smembers(a[0]!) };
    }
    case "SISMEMBER": {
    ...
    case "SCARD": {
      ...
      if (err) return { error: err };
      return { result: await await store.scard(a[0]!) };
    }
    case "SPOP": {
      ...
      if (a.length > 1) {
        const count = toInt(a[1]!);
        if (count === null) return { error: "ERR value is not an integer or out of range" };
        return { result: await await store.spop(a[0]!, count) };
      }
      return { result: await await store.spop(a[0]!) };
    }
    case "SRANDMEMBER": {
      ...
      if (a.length > 1) {
        const count = toInt(a[1]!);
        if (count === null) return { error: "ERR value is not an integer or out of range" };
        return { result: await await store.srandmember(a[0]!, count) };
      }
      return { result: await await store.srandmember(a[0]!) };
    }
    case "SUNION": {
      const err = minArgs(a, 1, "sunion");
      if (err) return { error: err };
      return { result: await await store.sunion(a) };
    }
    case "SINTER": {
      const err = minArgs(a, 1, "sinter");
      if (err) return { error: err };
      return { result: await await store.sinter(a) };
    }
    case "SDIFF": {
      const err = minArgs(a, 1, "sdiff");
      if (err) return { error: err };
      return { result: await await store.sdiff(a) };
    }
    case "SUNIONSTORE": {
      const err = minArgs(a, 2, "sunionstore");
      if (err) return { error: err };
      return { result: await await store.sunionstore(a[0]!, a.slice(1)) };
    }
    case "SINTERSTORE": {
      const err = minArgs(a, 2, "sinterstore");
      if (err) return { error: err };
      return { result: await await store.sinterstore(a[0]!, a.slice(1)) };
    }
    case "SDIFFSTORE": {
      const err = minArgs(a, 2, "sdiffstore");
      if (err) return { error: err };
      return { result: await await store.sdiffstore(a[0]!, a.slice(1)) };
    }
    case "SSCAN": {
      ...
      if (cursor === null) return { error: "ERR invalid cursor" };
      const opts = parseScanOptions(a.slice(2));
      const [next, items] = await store.sscan(a[0]!, cursor, opts);
      return { result: [String(next), items] };
    }
    ...
    case "ZADD": {
      ...
      return { result: await await store.zadd(a[0]!, entries, opts) };
    }
    case "ZREM": {
      const err = minArgs(a, 2, "zrem");
      if (err) return { error: err };
      return { result: await await store.zrem(a[0]!, a.slice(1)) };
    }
    case "ZSCORE": {
      const err = exactArgs(a, 2, "zscore");
      if (err) return { error: err };
      const score = await store.zscore(a[0]!, a[1]!);
      return { result: score !== null ? String(score) : null };
    }
    case "ZRANK": {
      const err = exactArgs(a, 2, "zrank");
      if (err) return { error: err };
      return { result: await await store.zrank(a[0]!, a[1]!) };
    }
    case "ZREVRANK": {
      const err = exactArgs(a, 2, "zrevrank");
      if (err) return { error: err };
      return { result: await await store.zrevrank(a[0]!, a[1]!) };
    }
    case "ZRANGE": {
      ...
      return { result: await await store.zrange(a[0]!, start, stop, withScores) };
    }
    case "ZRANGEBYSCORE": {
      const err = minArgs(a, 3, "zrangebyscore");
      if (err) return { error: err };
      const opts = parseZRangeByScoreOptions(a.slice(3));
      return { result: await await store.zrangebyscore(a[0]!, a[1]!, a[2]!, opts) };
    }
    case "ZREVRANGE": {
      ...
      if (start === null || stop === null)
        return { error: "ERR value is not an integer or out of range" };
      const withScores = a.length > 3 && a[3]!.toUpperCase() === "WITHSCORES";
      return { result: await await store.zrevrange(a[0]!, start, stop, withScores) };
    }
    case "ZREVRANGEBYSCORE": {
      const err = minArgs(a, 3, "zrevrangebyscore");
      if (err) return { error: err };
      const opts = parseZRangeByScoreOptions(a.slice(3));
      return { result: await await store.zrevrangebyscore(a[0]!, a[1]!, a[2]!, opts) };
    }
    case "ZCARD": {
      const err = exactArgs(a, 1, "zcard");
      if (err) return { error: err };
      return { result: await await store.zcard(a[0]!) };
    }
    case "ZCOUNT": {
      const err = exactArgs(a, 3, "zcount");
      if (err) return { error: err };
      return { result: await await store.zcount(a[0]!, a[1]!, a[2]!) };
    }
    case "ZINCRBY": {
      ...
      const inc = toFloat(a[1]!);
      if (inc === null) return { error: "ERR value is not a valid float" };
      return { result: await await store.zincrby(a[0]!, inc, a[2]!) };
    }
    case "ZPOPMIN": {
      ...
      if (count === null) return { error: "ERR value is not an integer or out of range" };
      return { result: await await store.zpopmin(a[0]!, count) };
    }
    case "ZPOPMAX": {
      ...
      const count = a.length > 1 ? toInt(a[1]!) : 1;
      if (count === null) return { error: "ERR value is not an integer or out of range" };
      return { result: await await store.zpopmax(a[0]!, count) };
    }
    case "ZUNIONSTORE": {
      ...
      return { result: await await store.zunionstore(a[0]!, keys, weights ?? undefined) };
    }
    case "ZINTERSTORE": {
      ...
      const rest = a.slice(2 + numkeys);
      const weights = parseWeights(rest);
      return { result: await await store.zinterstore(a[0]!, keys, weights ?? undefined) };
    }
    case "ZSCAN": {
      ...
      if (cursor === null) return { error: "ERR invalid cursor" };
      const opts = parseScanOptions(a.slice(2));
      const [next, items] = await store.zscan(a[0]!, cursor, opts);
      return { result: [String(next), items] };
    }
